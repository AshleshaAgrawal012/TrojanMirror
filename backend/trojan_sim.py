...
def _reset_sandbox():
    """Restores the victim_data directory to its initial plaintext state before simulation."""
    if not VICTIM_DIR.exists():
        VICTIM_DIR.mkdir(parents=True, exist_ok=True)
    
    # Remove existing files (including .encrypted ones)
    ...
    # Recreate pristine dummy files
    with open(VICTIM_DIR / "passwords.txt", "w") as f:
        f.write("admin:hunter2\nroot:toor123\n")
    with open(VICTIM_DIR / "project_notes.txt", "w") as f:
        f.write("Important details about the Trojan project.\nDo not lose this file.\n")
    with open(VICTIM_DIR / "personal_diary.txt", "w") as f:
...
def _run_simulation(captured_data):
    ...
    # Mask card for display (show last 4 digits only)
    card_masked = "****" + card_number[-4:] if len(card_number) >= 4 else "****"

    # Broadcast captured credentials to the dashboard
